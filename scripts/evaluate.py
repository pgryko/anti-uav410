...
def test(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                ...
                for p, b in zip(pred.tolist(), box.tolist(), strict=False):
                    jdict.append(
                        {
    ...
    # Compute statistics
    stats = [np.concatenate(x, 0) for x in zip(*stats, strict=False)]  # to numpy
    if len(stats) and stats[0].any():
        p, r, ap, f1, ap_class = ap_per_class(
            *stats, plot=plots, fname=save_dir / "precision-recall_curve.png"
        )
        # p, r, ap, f1, ap_class = ap_per_class(*stats, plot=plots,save_dir=save_dir, names='drone')
