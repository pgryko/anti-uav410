import torch
from trackinguav.features import deep
from trackinguav.features.extractor import MultiResolutionExtractor
from trackinguav.utils import FeatureParams, TrackerParams
...
def parameters():
    ...
    params.CG_iter = 2  # 5                  # The number of Conjugate Gradient iterations in each update after the first frame
    params.init_CG_iter = (
        100  # The total number of Conjugate Gradient iterations used in the first frame
    )
    ...
    params.standard_alpha = (
        True  # Use the standard formula for computing the step length in Conjugate Gradient
    )
    ...
    params.train_skipping = 20  # 10          # How often to run training (every n-th frame)

    # Detection parameters
    params.scale_factors = (
        1.02 ** torch.arange(-2, 3).float()
    )  # What scales to use for localization
    ...
    params.update_projection_matrix = (
        True  # Whether the projection matrix should be optimized or not
    )
    ...
    params.interpolation_windowing = (
        False  # Do additional windowing on the Fourier coefficients of the kernel
    )
    ...
    shallow_params.reg_window_power = (
        2  # The degree of the polynomial to use (e.g. 2 is a quadratic window)
    )
