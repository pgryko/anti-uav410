...
def rearrange(img):
    ...
    if img_[0:yh, 0:xh].shape == img[yh : img.shape[0], xh : img.shape[1]].shape:
        ...
        img_[0 : (yh + 1), 0:xh], img_[(yh + 1) : img.shape[0], xh : img.shape[1]] = (
            img[yh : img.shape[0], xh : img.shape[1]],
            img[0:yh, 0:xh],
        )
    ...
    if img_[0:yh, xh : img.shape[1]].shape == img[yh : img.shape[0], 0:xh].shape:
        ...
        img_[0 : (yh + 1), xh : img.shape[1]], img_[(yh + 1) : img.shape[0], 0:xh] = (
            img[yh : img.shape[0], 0:xh],
            img[0:yh, xh : img.shape[1]],
        )
    return img_
...
def subwindow(img, window, borderType=cv2.BORDER_CONSTANT):
    ...
    res = img[
        cutWindow[1] : cutWindow[1] + cutWindow[3], cutWindow[0] : cutWindow[0] + cutWindow[2]
    ]
...
class CorrFilterTracker:
    def __init__(self, hog=False, fixed_window=True, multiscale=False):
        ...
        if multiscale:
            ...
            self.scale_weight = (
                0.96  # to downweight detection scores of other scales for added stability
            )
        elif fixed_window:
    ...
    def gaussianCorrelation(self, x1, x2):
        ...
        if x1.ndim == 3 and x2.ndim == 3:
            d = (
                np.sum(x1[:, :, 0] * x1[:, :, 0]) + np.sum(x2[:, :, 0] * x2[:, :, 0]) - 2.0 * c
    ...
    def getFeatures(self, image, inithann, scale_adjust=1.02):
        ...
        if self._hogfeatures:
            ...
            FeaturesMap = (
                mapp["map"].reshape((self.size_patch[0] * self.size_patch[1], self.size_patch[2])).T
