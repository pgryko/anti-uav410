...
class Video:
    ...
    def draw_box(self, roi, img, linewidth, color, name=None):
        ...
        if len(roi) > 6 and len(roi) % 2 == 0:
            ...
            if not np.isnan(roi[0]):
                ...
                color = tuple(map(int, color))
                img = cv2.rectangle(
                    img, (roi[0], roi[1]), (roi[0] + roi[2], roi[1] + roi[3]), color, linewidth
                )
                if name:
                    img = cv2.putText(
                        img, name, (roi[0], roi[1] - 5), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, color, 1
                    )
        return img
    ...
    def show(self, pred_trajs={}, linewidth=2, show_name=False):
        ...
        for i, (roi, img) in enumerate(
            zip(self.gt_traj, self.imgs[self.start_frame : self.end_frame + 1], strict=False)
        ):
            img = img.copy()
            if len(img.shape) == 2:
