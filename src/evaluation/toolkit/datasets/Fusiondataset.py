...
class Light_video:
    ...
    def __init__(self, video_id, video_path, model, video_gt_real):
        ...
        list_imagename = [
            os.path.join(video_path, self.model, x)
            for x in os.listdir(os.path.join(self.video_path, self.model))
        ]
        self.image_names = sorted(
            list_imagename, key=lambda x: int(x.split(f"{model}I")[-1].strip(".jpg"))
        )
...
def read(number, model, root, name=None):
    ...
    if name:
        ...
        json_path = os.path.join(path_video_name, f"{model}.json")
