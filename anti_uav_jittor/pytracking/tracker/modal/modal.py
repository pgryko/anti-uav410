...
class Modal_tracker(SiameseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        x_crop = self.get_subwindow(
            image, self.center_pos, cfg.TRACK.INSTANCE_SIZE, round(s_x), self.channel_average
        )
        x_crop = x_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        x_crop[0] = jt.nn.normalize(x_crop[0], self.mean, self.std, self.inplace)
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        z_crop = self.get_subwindow(
            image, self.center_pos, cfg.TRACK.EXEMPLAR_SIZE, s_z, self.channel_average
        )
