...
from ltr_torch.models.backbone.resnet18_vggm import resnet18_vggmconv1
...
normalize = transform.image_normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
class ResNet18m1(MultiFeatureBase):
    ...
    def __init__(self, output_layers, net_path=None, use_gpu=True, *args, **kwargs):
        ...
        self.net_path = (
            "resnet18_vggmconv1/resnet18_vggmconv1.pth" if net_path is None else net_path
        )
    ...
    def initialize(self):
        ...
        self.mean = jt.Var([0.485, 0.456, 0.406]).view(1, -1, 1, 1)
        self.std = jt.Var([0.229, 0.224, 0.225]).view(1, -1, 1, 1)
        ...
        for net_path in net_path_full:
            try:
                ...
                pass
        if self.net is None:
            raise Exception(f"Did not find network file {self.net_path}")
    ...
    def stride(self):
        return TensorList(
            [
                ...
                for l, s in zip(self.output_layers, self.pool_stride, strict=False)
            ]
        )
...
class ATOMResNet18(MultiFeatureBase):
    ...
    def __init__(
        self, output_layers=("layer3",), net_path="atom_iou", use_gpu=True, *args, **kwargs
    ):
