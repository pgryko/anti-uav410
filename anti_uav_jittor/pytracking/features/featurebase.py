...
class FeatureBase:
    ...
    def size(self, im_sz):
        ...
        if isinstance(im_sz, jt.Var):
            return jt.Var([self.output_size[0], self.output_size[1]])
    ...
    def get_feature(self, im: jt.Var):
        """Get the feature. Generally, call this function.
        args:
            im: image patch as a jt.Var.
        """
        ...
        if self.output_size is not None:
            feat = jt.nn.AdaptiveAvgPool2d(feat, self.output_size)
        elif self.pool_stride != 1:
            feat = jt.nn.avg_pool2d(feat, self.pool_stride, self.pool_stride)

        # Normalize
        if self.normalize_power is not None:
            feat /= (
                jt.sum(
                    feat.abs().view(feat.shape[0], 1, 1, -1) ** self.normalize_power,
                    dim=3,
                    keepdim=True,
                )
...
class MultiFeatureBase(FeatureBase):
    ...
    def size(self, im_sz):
        ...
        if isinstance(im_sz, jt.Var):
            return TensorList(
                [
                    im_sz // s if sz is None else jt.Var([sz[0], sz[1]])
                    for sz, s in zip(self.output_size, self.stride(), strict=False)
                ]
            )
    ...
    def get_feature(self, im: jt.Var):
        ...
        # Pool/downsample
        for i, (sz, s) in enumerate(zip(output_sz, self.pool_stride, strict=False)):
            if sz is not None:
                feat_list[i] = jt.nn.AdaptiveAvgPool2d(feat_list[i], sz)
            elif s != 1:
                feat_list[i] = jt.nn.avg_pool2d(feat_list[i], s, s)
