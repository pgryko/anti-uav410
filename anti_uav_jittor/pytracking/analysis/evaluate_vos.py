...
import numpy as np
import pandas as pd
from ltr_torch.data.image_loader import imread_indexed
...
def evaluate_sequence(seq_name, segmentations, annotations, object_info, measure="J"):
    ...
    for obj_id, first_frame in object_info.items():
        ...
        for i, (an, sg) in enumerate(zip(annotations, segmentations, strict=False)):
...
def evaluate_dataset(
    results_path, dset_name, measure="J", to_file=True, scores=False, sequences=None, quiet=False
):
    ...
    for j, sequence in enumerate(dset):
        ...
        _print(
            "%d/%d: %s: %d object%s" % (j + 1, n_seqs, seq_name, n_objs, "s" if n_objs > 1 else "")
        )
        ...
        seq_mean_score = utils.nanmean(
            np.array(per_frame_score), axis=0
        )  # Mean object score per frame

        # Print sequence results
        _print(
            f"final  : acc {seq_score:.3f} ({np.mean(dset_scores):.3f}) ┊{utils.text_bargraph(seq_mean_score)}┊"
        )
...
def evaluate_vos(trackers, dataset="yt2019_jjval", force=False):
    ...
    for t in trackers:
        if t.display_name is not None:
            ...
            disp_name = f"{t.name} {t.parameter_name}_{t.run_id:03d}"
        else:
        ...
        if (
            os.path.exists(csv_name_global_path)
            and os.path.exists(csv_name_per_sequence_path)
            and not force
        ):
            ...
            g_res = np.array(
                [utils.mean(dset_scores), utils.mean(dset_recall), utils.mean(dset_decay)]
            )
            ...
            table_seq = pd.DataFrame(
                data=list(zip(seq_names, dset_scores, dset_recall, dset_decay, strict=False)),
