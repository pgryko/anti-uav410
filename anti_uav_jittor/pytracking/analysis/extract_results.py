...
def calc_iou_overlap(pred_bb, anno_bb):
    tl = jt.max(pred_bb[:, :2], anno_bb[:, :2])
    br = jt.min(pred_bb[:, :2] + pred_bb[:, 2:] - 1.0, anno_bb[:, :2] + anno_bb[:, 2:] - 1.0)
...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    if jt.isnan(pred_bb).any() or (pred_bb[:, 2:] < 0.0).any():
        raise Exception("Error: Invalid results")

    if jt.isnan(anno_bb).any():
        if dataset == "uav":
            pass
        else:
    ...
    if (pred_bb[:, 2:] == 0.0).any():
        for i in range(1, pred_bb.shape[0]):
            if (pred_bb[i, 2:] == 0.0).any() and not jt.isnan(anno_bb[i, :]).any():
    ...
    if pred_bb.shape[0] != anno_bb.shape[0]:
        if dataset == "lasot":
            ...
            if pred_bb.shape[0] > anno_bb.shape[0]:
                ...
                pad = jt.Var.zeros((anno_bb.shape[0] - pred_bb.shape[0], 4)).type_as(pred_bb)
                pred_bb = jt.concat((pred_bb, pad), dim=0)
...
def extract_results(
    ...
    threshold_set_overlap = jt.arange(0.0, 1.0 + plot_bin_gap, plot_bin_gap, dtype=jt.float64)
    threshold_set_center = jt.arange(0, 51, dtype=jt.float64)
    threshold_set_center_norm = jt.arange(0, 51, dtype=jt.float64) / 100.0

    avg_overlap_all = jt.Var.zeros((len(dataset), len(trackers)), dtype=jt.float64)
    ave_success_rate_plot_overlap = jt.Var.zeros(
        (len(dataset), len(trackers), threshold_set_overlap.numel()), dtype=jt.float32
    )
    ave_success_rate_plot_center = jt.Var.zeros(
        (len(dataset), len(trackers), threshold_set_center.numel()), dtype=jt.float32
    )
    ave_success_rate_plot_center_norm = jt.Var.zeros(
        (len(dataset), len(trackers), threshold_set_center.numel()), dtype=jt.float32
    )

    valid_sequence = jt.ones(len(dataset), dtype=jt.uint8)

    for seq_id, seq in enumerate(tqdm(dataset)):
        # Load anno
        anno_bb = jt.Var(seq.ground_truth_rect)
        target_visible = (
            jt.Var(seq.target_visible, dtype=jt.uint8) if seq.target_visible is not None else None
        )
