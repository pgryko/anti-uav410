...
class Tracker:
    ...
    def __init__(
        self, name: str, parameter_name: str, run_id: int = None, display_name: str = None
    ):
        ...
        if self.run_id is None:
            ...
            self.segmentation_dir = f"{env.segmentation_path}/{self.name}/{self.parameter_name}"
        else:
            self.results_dir = (
                f"{env.results_path}/{self.name}/{self.parameter_name}_{self.run_id:03d}"
            )
            self.segmentation_dir = (
                f"{env.segmentation_path}/{self.name}/{self.parameter_name}_{self.run_id:03d}"
            )
        ...
        if os.path.isdir(tracker_module_abspath):
            tracker_module = importlib.import_module(f"pytracking.tracker.{self.name}")
    ...
    def run_sequence(
        self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None
    ):
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        output = {"target_bbox": [], "time": [], "segmentation": []}
    ...
    def run_video(
        self, videofilepath, optional_box=None, debug=None, visdom_info=None, save_results=False
    ):
        ...
        assert os.path.isfile(videofilepath), f"Invalid param {videofilepath}"
        ...
        while True:
            ...
            font_color = (0, 0, 0)
            cv.putText(
                frame_disp, "Tracking!", (20, 30), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, font_color, 1
            )
            cv.putText(
                frame_disp,
                "Press r to reset",
    ...
    def run_vot2020(self, debug=None, visdom_info=None):
        ...
        if output_segmentation:
            ...
            bbox = masks_to_bboxes(jt.Var(vot_anno_mask), fmt="t").squeeze().tolist()
        else:
    ...
    def get_parameters(self):
        ...
        param_module = importlib.import_module(
            f"pytracking.parameter.{self.name}.{self.parameter_name}"
        )
