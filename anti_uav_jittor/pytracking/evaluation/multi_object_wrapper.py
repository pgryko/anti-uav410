...
class MultiObjectWrapper:
    ...
    def _split_info(self, info):
        ...
        for obj_id in info["init_object_ids"]:
            ...
            if "init_mask" in info:
                info_split[obj_id]["init_mask"] = (info["init_mask"] == int(obj_id)).astype(
                    np.uint8
                )
    ...
    def default_merge(self, out_all):
        ...
        if "segmentation" in out_types and out_first["segmentation"] is not None:
            ...
            segmentation_maps = [
                out.get("segmentation_soft", out["segmentation"]) for out in out_all.values()
            ]
            ...
            merged_segmentation = obj_ids[
                np.where(
                    segmentation_maps.max(axis=0) > segm_threshold,
                    segmentation_maps.argmax(axis=0) + 1,
                    0,
                )
            ]
