...
dataset_dict = dict(
    ...
    tpl_nootb=DatasetInfo(
        module=pt % "tpl", class_name="TPLDataset", kwargs=dict(exclude_otb=True)
    ),
    ...
    got10k_test=DatasetInfo(
        module=pt % "toolkit", class_name="GOT10KDataset", kwargs=dict(split="test")
    ),
    got10k_val=DatasetInfo(
        module=pt % "toolkit", class_name="GOT10KDataset", kwargs=dict(split="val")
    ),
    got10k_ltrval=DatasetInfo(
        module=pt % "toolkit", class_name="GOT10KDataset", kwargs=dict(split="ltrval")
    ),
    lasot=DatasetInfo(module=pt % "lasot", class_name="LaSOTDataset", kwargs=dict()),
    dv2017_val=DatasetInfo(
        module="ltr.dataset.davis", class_name="Davis", kwargs=dict(version="2017", split="val")
    ),
    dv2016_val=DatasetInfo(
        module="ltr.dataset.davis", class_name="Davis", kwargs=dict(version="2016", split="val")
    ),
    dv2017_test_dev=DatasetInfo(
        module="ltr.dataset.davis",
        class_name="Davis",
