...
def hann2d_clipped(sz: jt.Var, effective_sz: jt.Var, centered=True) -> jt.Var:
    ...
    window = jt.nn.pad(
        effective_window, (pad[1].item(), pad[1].item(), pad[0].item(), pad[0].item()), "replicate"
    )
...
def gauss_fourier(sz: int, sigma: float, half: bool = False) -> jt.Var:
    ...
    return (math.sqrt(2 * math.pi) * sigma / sz) * jt.exp(
...
def label_function_spatial(
    sz: jt.Var, sigma: jt.Var, center: jt.Var = jt.zeros(2), end_pad: jt.Var = jt.zeros(2)
):
    """The origin is in the middle of the image."""
    return gauss_spatial(sz[0].item(), sigma[0].item(), center[0], end_pad[0].item()).reshape(
        1, 1, -1, 1
    ) * gauss_spatial(sz[1].item(), sigma[1].item(), center[1], end_pad[1].item()).reshape(
        1, 1, 1, -1
    )
...
def cubic_spline_fourier(f, a):
    ...
    bf = (
        6 * (1 - jt.cos(2 * math.pi * f))
        + 3 * a * (1 - jt.cos(4 * math.pi * f))
        - (6 + 8 * a) * math.pi * f * jt.sin(2 * math.pi * f)
        - 2 * a * math.pi * f * jt.sin(4 * math.pi * f)
    ) / (4 * math.pi**4 * f**4)
...
def get_interp_fourier(
    sz: jt.Var, method="ideal", bicubic_param=0.5, centering=True, windowing=False, device="cpu"
):
...
def get_reg_filter(sz: jt.Var, target_sz: jt.Var, params):
    ...
    if getattr(params, "reg_window_centered", True):
        wrg = jt.arange(-int((sz[0] - 1) / 2), int(sz[0] / 2 + 1), dtype=jt.float32).view(
            1, 1, -1, 1
        )
        wcg = jt.arange(-int((sz[1] - 1) / 2), int(sz[1] / 2 + 1), dtype=jt.float32).view(
            1, 1, 1, -1
        )
    else:
        wrg = jt.concat(
            [
                jt.arange(0, int(sz[0] / 2 + 1), dtype=jt.float32),
                jt.arange(-int((sz[0] - 1) / 2), 0, dtype=jt.float32),
            ]
        ).view(1, 1, -1, 1)
        wcg = jt.concat(
            [
                jt.arange(0, int(sz[1] / 2 + 1), dtype=jt.float32),
                jt.arange(-int((sz[1] - 1) / 2), 0, dtype=jt.float32),
            ]
        ).view(1, 1, 1, -1)
    ...
    reg_window = (params.reg_window_edge - params.reg_window_min) * (
        ...
        + jt.abs(wcg / reg_scale[1]) ** params.reg_window_power
    ...
    reg_window_dft[
        reg_window_dft_abs < params.reg_sparsity_threshold * reg_window_dft_abs.max(), :
    ] = 0
...
def max2d(a: jt.Var):
    ...
    argmax_row = argmax_row.view(argmax_col.numel(), -1)[
        jt.arange(argmax_col.numel()), argmax_col.view(-1)
    ]
