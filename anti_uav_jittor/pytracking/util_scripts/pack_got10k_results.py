...
def pack_got10k_results(tracker_name, param_name, output_name):
    ...
    for i in range(1, 181):
        seq_name = f"GOT-10k_Test_{i:06d}"

        seq_output_path = f"{output_path}/{seq_name}"
        ...
        for run_id in range(1):
            ...
            times = np.loadtxt(
                f"{results_path}/{tracker_name}/{param_name}_{run_id:03d}/{seq_name}_time.txt",
                dtype=np.float64,
            )

            np.savetxt(
                f"{seq_output_path}/{seq_name}_{run_id+1:03d}.txt", res, delimiter=",", fmt="%f"
            )
            np.savetxt(f"{seq_output_path}/{seq_name}_time.txt", times, fmt="%f")
