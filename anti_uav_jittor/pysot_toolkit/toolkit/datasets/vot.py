...
class VOTVideo(Video):
    ...
    def __init__(
        ...
        super(VOTVideo, self).__init__(
            name, root, video_dir, init_rect, img_names, gt_rect, None, load_img
        )
...
class VOTDataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        ...
        with open(os.path.join("pysot_toolkit/testing_dataset", name + ".json")) as f:
...
class VOTLTVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names, gt_rect, load_img=False):
        super(VOTLTVideo, self).__init__(
            name, root, video_dir, init_rect, img_names, gt_rect, None, load_img
        )
    ...
    def load_tracker(self, path, tracker_names=None, store=True):
        ...
        for name in tracker_names:
            ...
            confidence_file = os.path.join(
                path, name, "longterm", self.name, self.name + "_001_confidence.value"
            )
...
class VOTLTDataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        ...
        with open(
            "D:\\study\\track\\TransT-main\\TransT-main\\pysot_toolkit\\testing_dataset\\VOT2018.json"
        ) as f:
            meta_data = json.load(f)
...
# dataset_root = 'D:\study\\track\VOT2018'
# dataset = VOTLTDataset('VOT2018',dataset_root=dataset_root,load_img=False)
# print('hello')
