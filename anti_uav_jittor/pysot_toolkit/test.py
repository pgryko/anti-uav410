...
for v_idx in range(len(test_dataset)):
    ...
    videoWriter = cv2.VideoWriter(f"test{rgb_video.video_id}_or.avi", fourcc, 30, (640 * 2, 512))
    toc = 0
    retemplate = False
    track_times = []
    video_acc_rgb = []
    ...
    record_file_ir = os.path.join("result", "UNFPN_IR", "%s.txt" % rgb_video.video_id)
    record_file_rgb = os.path.join("result", "UNFPN_RGB", "%s.txt" % rgb_video.video_id)
