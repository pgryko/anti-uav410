...
class Tracker:
    ...
    def initialize(self, image, info: dict, model) -> dict:
        ...
        z_crop = self.get_subwindow(
            image, center_pos, self.exemplar_size, s_z, self.channel_average
        )
    ...
    def track(self, images):
        ...
        #            }
        #     return out,out
        x_crop_ir = self.get_subwindow(
            image_ir, self.center_pos_ir, self.instance_size, round(s_x_ir), self.channel_average
        )

        x_crop_rgb = self.get_subwindow(
            image_rgb, self.center_pos_rgb, self.instance_size, round(s_x_rgb), self.channel_average
        )
    ...
    def finnal_out(self, out, mode, image):
        ...
        fout = {"target_bbox": [bbox], "best_score": pscore[best_idx]}
        return fout
    ...
    def re_dectect(self, image, mode):
        ...
        if top_conf[0] == 0:
            out = {"target_bbox": [0, 0, 0, 0], "best_score": 0}
        ...
        for i in range(len(top_conf)):
            ...
            top_boxes[i] = [(cx - width / 2) * w, (cy - height / 2) * h, width * w, height * h]
        best_id = np.argmax(top_conf)
        out = {"target_bbox": top_boxes, "best_score": top_conf[best_id]}
        return out
    ...
    def track_multi_modal(self, images):
        ...
        distance = np.linalg.norm(
            center_ir / np.array([hir, wir]) - center_rgb / np.array([hrgb, wrgb])
        )
        ...
        if isinstance(result_ir_bbox, int) or isinstance(result_rgb_bbox, int):  # 其中一个为空
            if not isinstance(result_rgb_bbox, int):  # rgb不为空，ir为空
                self.center_pos_ir = np.array(
                    [
                        (result_rgb_bbox[0] + result_rgb_bbox[2] / 2) / x_factor,
                        (result_rgb_bbox[1] + result_rgb_bbox[3] / 2) / y_factor,
                    ]
                )
            ...
            self.center_pos_ir = np.array(
                [
                    result_ir_bbox[0] + result_ir_bbox[2] / 2,
                    result_ir_bbox[1] + result_ir_bbox[3] / 2,
                ]
            )
            self.size_ir = np.array([result_ir_bbox[2], result_ir_bbox[3]])
            self.center_pos_rgb = np.array(
                [
                    result_rgb_bbox[0] + result_rgb_bbox[2] / 2,
                    result_rgb_bbox[1] + result_rgb_bbox[3] / 2,
                ]
            )
    ...
    def track_only_local(self, images):
        ...
        self.center_pos_ir = np.array(
            [result_ir_bbox[0] + result_ir_bbox[2] / 2, result_ir_bbox[1] + result_ir_bbox[3] / 2]
        )
        self.size_ir = np.array([result_ir_bbox[2], result_ir_bbox[3]])
        self.center_pos_rgb = np.array(
            [
