...
class ExperimentAntiUAV410:
    ...
    def __init__(
        ...
        self.att_name = [
            ...
            "Fast Motion",
            "Occlusion",
            "Dynamic Background Clutter",
            "Tiny Size",
            "Small Size",
            "Medium Size",
            "Normal Size",
        ]
        self.att_fig_name = ["TC", "OV", "SV", "FM", "OC", "DBC", "TS", "SS", "MS", "NS"]
    ...
    def eval(self, out_res, label_res):
        ...
        for _pred, _gt, _exist in zip(
            out_res, label_res["gt_rect"], label_res["exist"], strict=False
        ):
            if not _exist:
    ...
    def plot_curves(self, tracker_names):
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                legends.append(
                    "$\\bf{Siam}$ $\\bf{R}$$\\bf{-}$$\\bf{CNN}$: [%.3f]"
                    % performance[name][key]["success_score"]
                )
            else:
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                legends.append(
                    "$\\bf{Siam}$ $\\bf{R}$$\\bf{-}$$\\bf{CNN}$: [%.3f]"
                    % performance[name][key]["precision_score"]
                )
            else:
    ...
    def plot_attcurves(self, tracker_names, att_name, att_key):
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                legends.append(
                    "$\\bf{Siam}$ $\\bf{R}$$\\bf{-}$$\\bf{CNN}$: [%.3f]"
                    % performance[name][att_key]["att_success_score"]
                )
            else:
                legends.append(
                    "%s: [%.3f]" % (name, performance[name][att_key]["att_success_score"])
                )
        # matplotlib.rcParams.update({'font.size': 15.0})
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                legends.append(
                    "$\\bf{Siam}$ $\\bf{R}$$\\bf{-}$$\\bf{CNN}$: [%.3f]"
                    % performance[name][att_key]["att_precision_score"]
                )
            else:
                legends.append(
                    "%s: [%.3f]" % (name, performance[name][att_key]["att_precision_score"])
                )
        # matplotlib.rcParams.update({'font.size': 7.4})
