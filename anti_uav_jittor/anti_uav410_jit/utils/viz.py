...
def show_frame(
    image, boxes=None, fig_n=1, pause=0.001, linewidth=3, cmap=None, colors=None, legends=None
):
    ...
    if fig_n not in fig_dict or fig_dict[fig_n].get_size() != image.size[::-1]:
    ...
    if boxes is not None:
        ...
        if fig_n not in patch_dict:
            ...
            for patch, box in zip(patch_dict[fig_n], boxes, strict=False):
        ...
        if legends is not None:
            fig_dict[fig_n].axes.legend(
                patch_dict[fig_n], legends, loc=1, prop={"size": 8}, fancybox=True, framealpha=0.5
            )
