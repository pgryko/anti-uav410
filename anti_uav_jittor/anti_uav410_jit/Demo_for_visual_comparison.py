...
def main(visulization=True):
    ...
    trackers = [
        ...
        {
            "name": "SiamBAN",
            "path": "./Tracking_results/Trained_with_antiuav410/SiamBAN",
            "mode": 1,
        },
        {
            "name": "SiamCAR",
            "path": "./Tracking_results/Trained_with_antiuav410/SiamCAR",
            "mode": 1,
        },
        {
            "name": "Stark",
    ...
    colors = [
        ...
        (255, 255, 0),
        (65, 98, 0),
    ]  # blue, red, yellow, purple, orange, light blue

    label_files = sorted(glob.glob(os.path.join(dataset_path, "*/IR_label.json")))
    ...
    for video_id, label_file in enumerate(label_files, start=1):
        ...
        for trackerid, tracker in enumerate(trackers, start=1):
            pred_file = os.path.join(tracker["path"], "%s.txt" % video_dirsbase)

            if tracker["name"] == "SwinTrack-Tiny":
                pred_file = os.path.join(
                    tracker["path"],
                    "test_metrics/anti-uav410-%s/%s/bounding_box.txt" % (subset, video_dirsbase),
                )
            try:
        ...
        img_files = sorted(glob.glob(os.path.join(dataset_path, video_dirsbase, "*.jpg")))
        ...
        for frame_id, img_file in enumerate(img_files):
            ...
            for trackerid, (Tbboxes, Tcolor) in enumerate(
                zip(pred_res_total, colors, strict=False), start=0
            ):
                bbox = Tbboxes[frame_id]
                cv2.putText(
                    frame, trackers[trackerid]["name"], (20, 60 + 30 * trackerid), 1, 2, Tcolor, 2
                )
                if len(bbox) > 3:
                    cv2.rectangle(
                        frame,
            ...
            cv2.putText(
                frame,
                video_dirsbase,
                (frame.shape[1] - 225, frame.shape[0] - 10),
                1,
                1,
                (255, 255, 0),
                2,
