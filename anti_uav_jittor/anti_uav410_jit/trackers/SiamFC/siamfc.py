...
class TrackerSiamFC:
    def __init__(self, net_path=None, input_type="image", color_fmt="RGB", **kargs):
        self.name = "SiamFC"
        ...
        if net_path is not None:
            self.net.load_state_dict(jt.load(net_path, map_location=lambda storage, loc: storage))
    ...
    def forward_test(self, img_files, init_bbox, visualize=False):
        ...
        for f, img_file in enumerate(img_files):
            if self.input_type == "image":
                img = image.read_image(img_file, self.color_fmt)
    ...
    def init(self, image, box):
        ...
        exemplar_image = self._crop_and_resize(
            image, self.center, self.z_sz, out_size=self.cfg.exemplar_sz, pad_color=self.avg_color
        )

        # exemplar features
        exemplar_image = (
            jt.from_numpy(exemplar_image).to(self.device).permute([2, 0, 1]).unsqueeze(0).float()
        )
    ...
    def update(self, image):
        ...
        instance_images = (
            torch.from_numpy(instance_images).to(self.device).permute([0, 3, 1, 2]).float()
        )
        ...
        responses = np.stack(
            [
                cv2.resize(t, (self.upscale_sz, self.upscale_sz), interpolation=cv2.INTER_CUBIC)
                for t in responses
            ],
            axis=0,
        )
