...
class Registry:
    ...
    def _init_pytorch_modules(self):
        ...
        for k, v in optim.LRScheduler.__dict__.items():
            if not isinstance(v, type):
                continue
            if issubclass(v, optim.LRScheduler) and v is not optim.LRScheduler:
                self.register_module(v, prefix)
        ...
        for k, v in jt.dataset.Sampler.__dict__.items():
            if not isinstance(v, type):
                continue
            if issubclass(v, jt.dataset.Sampler) and v is not jt.dataset.Sampler:
                self.register_module(v, prefix)
        ...
        for k, v in jt.dataset.__dict__.items():
            if not isinstance(v, type):
                continue
            if issubclass(v, jt.dataset.Dataset) and v is not jt.dataset.Dataset:
                self.register_module(v, prefix)
        self.register_module(jt.dataset.DataLoader, prefix)

    def register_module(self, module, prefix=""):
        if not inspect.isclass(module) and not inspect.isfunction(module):
            raise TypeError("module must be a class or a function, " f"but got {module}")
    ...
    def build(self, cfg):
        ...
        if isinstance(module_name, six.string_types):
            ...
            if module.__name__ == "dict":
                module = addict.Dict
            if module is None:
                raise KeyError(f"{module_name} is not in the registry")
        else:
            raise TypeError(f"type must be a string, but got {module_name}")
    ...
    def __repr__(self):
        repr_str = self.__class__.__name + f"(items={list(self._module_dict.keys())})"
        return repr_str
