...
def show_image(
    ...
    if bboxes is not None:
        ...
        cv2.putText(
            img, str(frameNo + 1), (20, 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (185, 218, 255), 2
        )

        if up_flag:
            cv2.putText(
                img, "update!!!", (200, 50), cv2.FONT_HERSHEY_COMPLEX, 2.0, (200, 100, 200), 5
            )
        else:
            cv2.putText(
                img, "no update!!!", (150, 50), cv2.FONT_HERSHEY_COMPLEX, 2.0, (200, 100, 200), 5
            )
    ...
    if visualize:
        if isinstance(fig, str):
            ...
            winname = f"window_{fig}"
        # cv2.imshow(winname, img)
        # cv2.waitKey(delay)
        viz.image(
            img.transpose(2, 0, 1)[::-1, ...], win=winname, opts={"title": "tracking results"}
        )
