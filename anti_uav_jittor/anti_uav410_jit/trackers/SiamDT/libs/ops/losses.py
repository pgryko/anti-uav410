...
def _log_sigmoid(x):
    ...
    return jt.clamp(x, max=0) - jt.log(1 + jt.exp(-jt.abs(x))) + 0.5 * jt.clamp(x, min=0, max=0)
...
def balanced_bce_loss(input, target, neg_weight=1.0):
    ...
    weight[neg_mask] = (1.0 / neg_num) * neg_weight
    weight /= weight.mean()
    return binary_cross_entropy_with_logits(input, target, weight, reduction="mean")
...
def focal_loss(input, target, alpha=0.25, gamma=2.0):
    ...
    weight = pt.pow(gamma) * (alpha * target + (1 - alpha) * (1 - target))

    # BCE loss with focal weights
    loss = binary_cross_entropy_with_logits(input, target, reduction="none") * weight
...
def ghmc_loss(input, target, bins=30, momentum=0.5):
    ...
    loss = binary_cross_entropy_with_logits(input, target, weight, reduction="sum") / total

    return loss


def ohem_bce_loss(input, target, neg_ratio=3.0):
    ...
    loss = binary_cross_entropy_with_logits(
        jt.concat([pos_logits, neg_logits]), jt.concat([pos_labels, neg_labels]), reduction="mean"
    )
...
def smooth_l1_loss(input, target, beta=1.0 / 9):
    ...
    loss = jt.where(o < beta, 0.5 * o**2 / beta, o - 0.5 * beta)
    return loss.mean()
...
def label_smooth_loss(input, target, num_classes, eps=0.1):
    ...
    target = log_probs.new_zeros(log_probs.size()).scatter_(1, target.unsqueeze(1), 1)
