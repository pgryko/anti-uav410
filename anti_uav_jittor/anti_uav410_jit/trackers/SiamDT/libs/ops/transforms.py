...
def make_pair(p):
    if isinstance(p, numbers.Number):
        ...
        raise ValueError(f"Unable to convert {p} to a pair")
...
def pad_img(img, shape, border_value=0):
    if len(shape) < len(img.shape):
        shape += (img.shape[-1],)
    assert all([so >= si for so, si in zip(shape, img.shape, strict=False)])
...
def jaccard(a, b):
    ...
    area_i = (rb - lt).clip(min=0.0).prod(axis=2)  # N x M

    area_a = (a[:, 2:] - a[:, :2]).prod(axis=1)  # N
    area_b = (b[:, 2:] - b[:, :2]).prod(axis=1)  # M
    ious = area_i / (area_a[:, None] + area_b[None, :] - area_i).clip(min=1e-6)
...
def random_crop(img, bboxes, min_ious, min_scale):
    ...
    while True:
        ...
        for i in range(50):
            ...
            ious = jaccard(bboxes.reshape(-1, 4), bound.reshape(-1, 4)).reshape(-1)
            if ious.min() < min_iou:
                continue
...
def brightness_(rng, img, gray, gray_mean, var):
    alpha = 1.0 + rng.uniform(low=-var, high=var)
    img *= alpha
...
def get_affine_transform(center, scale, rotation, output_size, shift=None, inv=False):
...
def apply_affine_transform(point, trans):
    out = np.array([point[0], point[1], 1.0], dtype=np.float32).T
...
def random_resize(img, max_scale):
    interp = random_interp()
    scale = np.random.uniform(1.0 / max_scale, max_scale)
...
def crop_and_resize(
    ...
    pads = np.concatenate((-corners[:2], corners[2:] - img.shape[1::-1]))
...
def crop_square(img, bbox, context, src_size, dst_size):
    ...
    interp = random_interp()
    patch = crop_and_resize(img, center, size, dst_size, border_value=avg_color, interp=interp)
...
def swin_crop_square(img, bbox, search_area_scale, dst_size):
    ...
    patch, ratio = swin_crop_and_resize(
        img, center, size, dst_size, border_value=avg_color, interp=interp
    )

    newtarget_sz = target_sz * ratio
    newbbox = [
        dst_size / 2 - (newtarget_sz[0] - 1) / 2,
        dst_size / 2 - (newtarget_sz[1] - 1) / 2,
        dst_size / 2 + (newtarget_sz[0] - 1) / 2,
        dst_size / 2 + (newtarget_sz[1] - 1) / 2,
    ]
