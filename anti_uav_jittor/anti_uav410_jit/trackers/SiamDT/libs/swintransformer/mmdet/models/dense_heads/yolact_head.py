...
class YOLACTHead(AnchorHead):
    ...
    def _get_bboxes_single(
        ...
        for cls_score, bbox_pred, coeff_pred, anchors in zip(
            cls_score_list, bbox_pred_list, coeff_preds_list, mlvl_anchors, strict=False
        ):
...
class YOLACTProtonet(nn.Module):
    ...
    def _init_layers(self):
        ...
        for num_channels, kernel_size in zip(
            self.proto_channels, self.proto_kernel_sizes, strict=False
        ):
            if kernel_size > 0:
    ...
    def get_seg_masks(self, mask_pred, label_pred, img_meta, rescale):
        ...
        for m, l in zip(mask_pred, label_pred, strict=False):
