...
class PAAHead(ATSSHead):
    ...
    def loss(
        ...
        (labels, labels_weight, bboxes_target, bboxes_weight, pos_inds, pos_gt_index) = (
            cls_reg_targets
        )
    ...
    def _get_bboxes(
        ...
        for cls_score, bbox_pred, iou_preds, anchors in zip(
            cls_scores, bbox_preds, iou_preds, mlvl_anchors, strict=False
        ):
        ...
        det_results = []
        for mlvl_bboxes, mlvl_scores in zip(batch_mlvl_bboxes, batch_mlvl_nms_scores, strict=False):
