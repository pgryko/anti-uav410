...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _get_bboxes(
        ...
        nms_pre_tensor = torch.tensor(
            cfg.get("nms_pre", -1), device=mlvl_cls_scores[0].device, dtype=torch.long
        )

        mlvl_bboxes = []
        mlvl_scores = []
        for cls_score, bbox_pred, anchors in zip(
            mlvl_cls_scores, mlvl_bbox_preds, mlvl_anchors, strict=False
        ):
        ...
        if with_nms:
            det_results = []
            for mlvl_bboxes, mlvl_scores in zip(batch_mlvl_bboxes, batch_mlvl_scores, strict=False):
            ...
            det_results = [
                tuple(mlvl_bs)
                for mlvl_bs in zip(batch_mlvl_bboxes, batch_mlvl_scores, strict=False)
            ]
        return det_results
