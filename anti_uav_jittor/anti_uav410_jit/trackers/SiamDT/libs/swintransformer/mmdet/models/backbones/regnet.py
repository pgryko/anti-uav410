...
class RegNet(ResNet):
    ...
    def adjust_width_group(self, widths, bottleneck_ratio, groups):
        ...
        """
        bottleneck_width = [int(w * b) for w, b in zip(widths, bottleneck_ratio, strict=False)]
        groups = [min(g, w_bot) for g, w_bot in zip(groups, bottleneck_width, strict=False)]
        bottleneck_width = [
            self.quantize_float(w_bot, g)
            for w_bot, g in zip(bottleneck_width, groups, strict=False)
        ]
        widths = [
            int(w_bot / b) for w_bot, b in zip(bottleneck_width, bottleneck_ratio, strict=False)
        ]
    ...
    def get_stages_from_blocks(self, widths):
        ...
        width_diff = [
            width != width_prev
            for width, width_prev in zip(widths + [0], [0] + widths, strict=False)
        ]
        stage_widths = [width for width, diff in zip(widths, width_diff[:-1], strict=False) if diff]
        stage_blocks = np.diff(
            [depth for depth, diff in zip(range(len(width_diff)), width_diff, strict=False) if diff]
        ).tolist()
