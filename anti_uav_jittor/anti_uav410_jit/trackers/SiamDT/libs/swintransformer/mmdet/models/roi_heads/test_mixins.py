...
class MaskTestMixin:
    ...
    def simple_test_mask(self, x, img_metas, det_bboxes, det_labels, rescale=False):
        ...
        if isinstance(det_bboxes, list):
            ...
            for i, (bbox, label) in enumerate(zip(det_bboxes, det_labels, strict=False)):
                supplement_bbox = bbox.new_full((max_size - bbox.size(0), bbox.size(1)), 0)
        ...
        # Recover the batch dimension
        mask_preds = mask_pred.reshape(batch_size, num_proposals_per_img, *mask_pred.shape[1:])
