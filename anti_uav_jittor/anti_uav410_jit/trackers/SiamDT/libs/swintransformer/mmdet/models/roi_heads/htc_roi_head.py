...
class HybridTaskCascadeRoIHead(CascadeRoIHead):
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if self.with_mask:
            results = list(
                zip(ms_bbox_result["ensemble"], ms_segm_result["ensemble"], strict=False)
            )
        else:
    ...
    def aug_test(self, img_feats, proposal_list, img_metas, rescale=False):
        ...
        aug_bboxes = []
        aug_scores = []
        for x, img_meta, semantic in zip(img_feats, img_metas, semantic_feats, strict=False):
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                for x, img_meta, semantic in zip(
                    img_feats, img_metas, semantic_feats, strict=False
                ):
                    img_shape = img_meta[0]["img_shape"]
                    scale_factor = img_meta[0]["scale_factor"]
                    flip = img_meta[0]["flip"]
