...
def test_translate():
    ...
    def check_translate(
        results, results_translated, offset, img_fill_val, seg_ignore_label, direction, min_size=0
    ):
        # check keys
    ...
    policies = [
        [
            ...
            dict(type="Translate", level=8, img_fill_val=img_fill_val, direction="vertical"),
        ]
    ]
    autoaug = dict(type="AutoAugment", policies=policies)
    autoaug_module = build_from_cfg(autoaug, PIPELINES)
