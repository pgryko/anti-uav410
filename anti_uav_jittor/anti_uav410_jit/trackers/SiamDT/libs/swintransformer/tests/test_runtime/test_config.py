...
def _check_mask_head(mask_cfg, mask_head):
    ...
    if isinstance(mask_cfg, list):
        for single_mask_cfg, single_mask_head in zip(mask_cfg, mask_head, strict=False):
...
def _check_bbox_head(bbox_cfg, bbox_head):
    ...
    if isinstance(bbox_cfg, list):
        for single_bbox_cfg, single_bbox_head in zip(bbox_cfg, bbox_head, strict=False):
