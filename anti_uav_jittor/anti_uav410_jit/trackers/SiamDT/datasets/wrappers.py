...
def _datasets(name):
    ...
    if name == "coco_train":
        ...
        return data.GOT10k(
            root_dir="/media/data2/TrackingDatasets/GOT-10k/full_data/train_data", subset="train"
        )
    elif name == "got10k_val":
        return data.GOT10k(
            root_dir="/media/data2/TrackingDatasets/GOT-10k/full_data/train_data", subset="val"
        )
    elif name == "lasot_train":
        return data.LaSOT(root_dir="/media/data2/TrackingDatasets/LaSOT/", subset="train")
    elif name == "uavtir_train":
        return data.UAVtir(
            root_dir="/media/data2/TrackingDatasets/Anti-UAV410/Anti-UAV/", subset="train"
        )
    elif name == "uavtir_val":
        return data.UAVtir(
            root_dir="/media/data2/TrackingDatasets/Anti-UAV410/Anti-UAV/", subset="val"
        )
...
def _transforms(name):
    ...
    if name == "basic_train":
        ...
        return data.ExtraPairTransforms(with_photometric=True, with_expand=False, with_crop=False)
...
class PairWrapper(data.PairDataset):
    ...
    def __getitem__(self, index):
        ...
        # sanity check
        keys = ["img_z", "img_x", "gt_bboxes_z", "gt_bboxes_x", "img_meta_z", "img_meta_x"]
        ...
        if self.max_instances > 0 and len(item["gt_bboxes_z"]) > self.max_instances:
            indices = np.random.choice(len(item["gt_bboxes_z"]), self.max_instances, replace=False)
    ...
    def _setup_base_dataset(self, base_dataset, base_transforms, sampling_prob, max_size):
