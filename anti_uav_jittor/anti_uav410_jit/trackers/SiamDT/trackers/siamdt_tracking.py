...
class SiamDTTracker(Tracker):
    def __init__(self, cfg_file, ckp_file, transforms, name_suffix="", visualize=False):
        ...
        if name_suffix:
            # name += '_' + name_suffix
            name = name_suffix
        super(SiamDTTracker, self).__init__(name=name, is_deterministic=True, visualize=visualize)
    ...
    def update(self, img, **kwargs):
        ...
        # get detections
        results, up_flag = self.model._process_gallary(img, [img_meta], rescale=True, **kwargs)
