...
from .similarity_encoders import RCNN_Similarity_Learning, RPN_Similarity_Learning
...
class SiamDTRCNN(TwoStageDetector):
    ...
    def forward_dummy(self, *args, **kwargs):
        raise NotImplementedError("forward_dummy is not implemented for SiamDTRCNN")
    ...
    def forward_train(
        ...
        template = [bbox_feats_z[rois_z[:, 0] == j] for j in range(len(gt_bboxes_z))]
        ...
        assign_result = bbox_assigner.assign(
            proposal_list[0], gt_bboxes_x[0], gt_bboxes_ignore[0], gt_labels[0]
        )
        ...
        bbox_targets = self.roi_head.bbox_head.get_targets(
            sampling_results, gt_bboxes_x, gt_labels, self.train_cfg.rcnn
        )
        ...
        **kwargs,
        ...
        if keep_order:
            det_bboxes, det_labels = self.roi_head.bbox_head.get_bboxes(
                rois_x, cls_score_x, bbox_pred_x, img_shape, scale_factor, rescale=rescale, cfg=None
            )
    ...
    def simple_matching(
        ...
        **kwargs,
        ...
        # similarity learning
        roi_feats = self.rcnn_similarity_learning(bbox_feats_z, bbox_feats_x)
        ...
        if keep_order:
            match_bboxes, match_labels = self.roi_head.bbox_head.get_bboxes(
                rois_x, cls_score, bbox_pred, img_shape, scale_factor, rescale=rescale, cfg=None
            )
    ...
    def aug_test(self, *args, **kwargs):
        raise NotImplementedError("aug_test is not implemented for SiamDTRCNN")

    def show_result(self, *args, **kwargs):
        raise NotImplementedError("show_result is not implemented for SiamDTRCNN")
    ...
    def _process_query(self, img_z, gt_bboxes_z, img_meta_z):
        ...
        self._template = [self._bbox_feats_z[rois_z[:, 0] == j] for j in range(len(gt_bboxes_z))]

        proposal_list = self.rpn_head.simple_test_rpn(z, img_meta_z)
    ...
    def _update_query(self, feat, proposal_list, gt_bboxes_z, img_meta_z):
        ...
        ) * self._bbox_feats_z + self._learning_rate * bbox_feats_z
    ...
    def _process_gallary(self, img_x, img_meta_x, **kwargs):
        ...
        # update flag
        if (
            tra_bboxes[0, -1] + det_bboxes[0, -1] > 1.9
            and self.computeiou(det_bboxes[0, :-1], tra_bboxes[0, :-1]) > 0.8
        ):
            up_flag = True
        ...
        if not kwargs.get("keep_order", False):
            ...
            bbox_results = [
                np.concatenate(
                    [newens_bboxes.cpu().numpy(), newens_labels.cpu().numpy()[:, None]], axis=1
                )
            ]
