...
def save_one_json(predn, jdict, path, class_map):
    ...
    for p, b in zip(predn.tolist(), box.tolist(), strict=False):
        jdict.append(
            {
                "image_id": image_id,
...
def run(
    ...
    # Compute metrics
    stats = [torch.cat(x, 0).cpu().numpy() for x in zip(*stats, strict=False)]  # to numpy
...
def parse_opt():
    ...
    parser.add_argument(
        "--data", type=str, default=ROOT / "data/antiuav.yaml", help="dataset.yaml path"
    )
    parser.add_argument(
        "--weights",
        nargs="+",
        type=str,
        default=ROOT / "runs/train/exp4/weights/best.pt",
        help="model path(s)",
    )
