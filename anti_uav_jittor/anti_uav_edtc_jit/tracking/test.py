...
def run_tracker(
    ...
    trackers = [
        Tracker(tracker_name, tracker_param, dataset_name, run_id, tracker_params=tracker_params)
    ]
...
def main():
    ...
    parser.add_argument(
        "--tracker_name", default="uavtrack_eh", type=str, help="Name of tracking method."
    )
    parser.add_argument(
        "--tracker_param", default="baseline", type=str, help="Name of config file."
    )
    ...
    parser.add_argument(
        "--params__model",
        type=str,
        default="/path/to/EDTC/pretrained_models/UAVTrackEH.pth.tar",
    ...
    tracker_params = {}
    for param in list(
        filter(lambda s: s.split("__")[0] == "params" and getattr(args, s) != None, args.__dir__())
    ):
