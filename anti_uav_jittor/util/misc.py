...
def all_gather(data):
    ...
    # tensor = jt.Var(storage, dtype = jt.uint8) # 与PyTorch不同，Jittor中没有显式的to方法来将张量移动到不同的设备，因为Jittor会自动管理张量的设备位置。

    # obtain Tensor size of each rank
    local_size = jt.var([tensor.numel()], device="cuda")
    size_list = [jt.var([0], device="cuda") for _ in range(world_size)]
    jt.all_gather(size_list, local_size)
    ...
    if local_size != max_size:
        ...
        tensor = jt.concat((tensor, padding), dim=0)
