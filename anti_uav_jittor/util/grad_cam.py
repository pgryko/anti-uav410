...
class BaseCAM:
    ...
    def forward(
        self, input_tensor: jt.Var, targets: list[jt.nn.Module], eigen_smooth: bool = False
    ) -> np.ndarray:
        if self.cuda:
        ...
        if self.uses_gradients:
            ...
            loss = sum([target(output) for target, output in zip(targets, outputs, strict=False)])
    ...
    def compute_cam_per_layer(
        self, input_tensor: jt.Var, targets: list[jt.nn.Module], eigen_smooth: bool
    ) -> np.ndarray:
...
def preprocess_image(img: np.ndarray, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]) -> jt.Var:
    preprocessing = transform.Compose(
        [transform.ToTensor(), transform.ImageNormalize(mean=mean, std=std)]
    )
