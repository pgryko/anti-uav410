...
from util.misc import NestedTensor, accuracy, nested_tensor_from_tensor, nested_tensor_from_tensor_2
...
class TransT(nn.Module):
    ...
    def forward(self, search, template):
        ...
        outputs_class = self.class_embed(hs)  # 1,4,256,2
        outputs_coord = self.bbox_embed(hs).sigmoid()  # 1,4,256,4
...
def transt_resnet50(settings):
    ...
    model = TransT(backbone_net, featurefusion_network, num_classes=num_classes)
    model
    return model
