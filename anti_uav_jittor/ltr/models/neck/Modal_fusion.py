...
class ModalFusionNetwork(nn.Module):
    ...
    def with_pos_embed(self, tensor, pos: jt.Var | None):
...
class ModalFusionMultiScale(jt.nn.Module):
    ...
    def execute(self, rgb, ir):
        ...
        for i in range(self.scale_layers):
            ...
            fusion_outi = self.fusionblock[i](
                rgb[2], ir[2], position_encodings_rgb, position_encodings_ir
            )
...
class FeatureFusionNetwork(nn.Module):
    def __init__(
        ...
        featurefusion_layer = FeatureFusionLayer(
            d_model, nhead, dim_feedexecute, dropout, activation
        )
    ...
    def execute(
        ...
        src_search,
        mask_search,
        pos_temp: jt.Var | None,
        pos_search: jt.Var | None,
    ):
...
class FeatureFusionNetwork_FPN(jt.nn.Module):
    def __init__(
        ...
        #                                          nhead=nhead,dim_feedexecute=dim_feedexecute, dropout=dropout, activation=activation)
        correlation3 = FeatureFusionNetwork(
            num_featurefusion_layers=1,
            d_model=d_model,
            nhead=nhead,
            dim_feedexecute=dim_feedexecute,
            dropout=dropout,
    ...
    def execute(self, src_search, src_temp):
        ...
        for i in range(self.multi_scales):
            ...
            """
            hs = correlation(
                src_temp[i], mask_temp, src_search[i], mask_search, positon_template, positon_search
            )
            out.append(hs)
...
class Decoder(nn.Module):
    ...
    def execute(
        ...
        tgt,
        memory,
        tgt_mask: jt.Var | None = None,
        memory_mask: jt.Var | None = None,
        tgt_key_padding_mask: jt.Var | None = None,
        memory_key_padding_mask: jt.Var | None = None,
        pos_enc: jt.Var | None = None,
        pos_dec: jt.Var | None = None,
    ):
        output = tgt
...
class Encoder(nn.Module):
    ...
    def execute(
        ...
        src1,
        src2,
        src1_mask: jt.Var | None = None,
        src2_mask: jt.Var | None = None,
        src1_key_padding_mask: jt.Var | None = None,
        src2_key_padding_mask: jt.Var | None = None,
        pos_src1: jt.Var | None = None,
        pos_src2: jt.Var | None = None,
    ):
        output1 = src1
        output2 = src2
...
class FeatureFusionLayer(nn.Module):
    ...
    def execute_post(
        ...
        src12 = self.self_attn1(
            q1, k1, value=src1, attn_mask=src1_mask, key_padding_mask=src1_key_padding_mask
        )[0]
        src1 = src1 + self.dropout11(src12)
        src1 = self.norm11(src1)

        q2 = k2 = self.with_pos_embed(src2, pos_src2)
        src22 = self.self_attn2(
            q2, k2, value=src2, attn_mask=src2_mask, key_padding_mask=src2_key_padding_mask
        )[0]
