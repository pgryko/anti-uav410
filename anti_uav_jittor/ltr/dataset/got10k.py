...
class Got10k(BaseVideoDataset):
    ...
    def __init__(
        self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None
    ):
        """
        args:
    ...
    def _read_target_visible(self, seq_path):
        ...
        with open(occlusion_file, newline="") as f:
            ...
            occlusion = jt.Var([int(v[0]) for v in csv.reader(f)], dtype=jt.uint8)
        with open(cover_file, newline="") as f:
            cover = jt.var([int(v[0]) for v in csv.reader(f)], dtype=jt.uint8)
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(seq_path, f"{frame_id+1:08}.jpg")  # frames start from 1
