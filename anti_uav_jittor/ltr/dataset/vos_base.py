...
class VOSMeta:
    ...
    def generate(cls, dset_name: str, dset_images_path: Path, dset_annos_path: Path):
        ...
        for seq in tqdm(sequences, desc=dset_name, unit="seq"):
            ...
            anno_paths = sorted((dset_annos_path / seq).glob("*.png"))
            ...
            for path in anno_paths:
                ...
                obj_sizes2[f_id] = OrderedDict(zip(obj_ids, obj_sizes, strict=False))
            ...
            dset_meta[seq] = dict(
                shape=shape, obj_sizes=obj_sizes2, bboxes=bboxes, frame_names=frame_names
            )
    ...
    def select_split(self, dataset_name, split):
        ...
        sequences = set(
            [
                s.strip()
                for s in open(
                    os.path.join(ltr_path, "data_specs", dataset_name + "_" + split + ".txt")
                ).readlines()
            ]
        )
    ...
    def enable_all_frames(self, dset_images_path):
        ...
        if idx_file.exists():
            ...
            for seq in self._data:
                all_frame_names[seq] = [
                    file.stem for file in sorted((dset_images_path / seq).glob("*.jpg"))
                ]
    ...
    def generate_datasets_meta(src, dst=Path("~/vosdataset_meta").expanduser()):
        VOSMeta.generate("SyntheticCoco", src / "JPEGImages", src / "Annotations").save(
            src / "generated_meta.json"
        )
...
class VOSDatasetBase(BaseVideoDataset):
    ...
    def get_sequence_info(self, sample_id):
        ...
        obj_sizes = jt.zeros((len(f_names), len(obj_ids)), dtype=jt.int32)
    ...
    def get_paths_and_bboxes(self, sequence_info):
        ...
        gt_labels = [
            str(annos_root / (f + ".png")) if f in all_bboxes.keys() else None for f in frame_names
        ]
    ...
    def get_sequence_list(self):
        ...
        self.sequence_list = [
            self._construct_sequence(self.get_sequence_info(i)) for i in range(len(self._samples))
        ]
        return self.sequence_list
    ...
    def get_frames(self, sample_id, frame_ids, anno=None):
        ...
        images = [
            self._load_image(self._jpeg_path / seq_name / (frame_names[f] + ".jpg"))
            for f in frame_ids
        ]
        labels = [
            self._load_anno(self._anno_path / seq_name / (frame_names[f] + ".png"))
            for f in frame_ids
        ]
        ...
        for bbox in bboxes:
            for obj_id in obj_ids:
                if obj_id not in bbox:
                    bbox[obj_id] = jt.zeros(4, dtype=jt.float32)

        # Remap to object id 1, if requested - for training
        if not self.multiobj:
            assert len(obj_ids) == 1
            obj_id = obj_ids[0]
            labels = [jt.var(lb == int(obj_id)) for lb in labels]
            bboxes = [bbox[obj_id] for bbox in bboxes]
        else:
            labels = [jt.var(lb) for lb in labels]

        object_meta = {
            key: meta[key] for key in ["sequence", "frame_shape", "frame_names", "object_ids"]
        }
