...
class AntiUav_fusion(BaseVideoDataset):
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        frame_name = model + "I" + f"{frame_id}".zfill(4) + ".jpg"
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        frame_list = {}
        frame_list["RGB"] = [
            self._get_frame(os.path.join(seq_path, "visible"), f_id) for f_id in frame_ids
        ]
        frame_list["IR"] = [
            self._get_frame(os.path.join(seq_path, "infrared"), f_id) for f_id in frame_ids
        ]
        anno_frames = {"RGB": {}, "IR": {}}
        if anno is None:
