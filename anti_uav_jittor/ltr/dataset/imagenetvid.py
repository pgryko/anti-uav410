...
def get_target_to_image_ratio(seq):
    anno = jt.var(seq["anno"])
    img_sz = jt.var(seq["image_size"])
...
class ImagenetVID(BaseVideoDataset):
    ...
    def __init__(
        self, root=None, image_loader=default_image_loader, min_length=0, max_target_area=1
    ):
        """
        args:
    ...
    def get_sequence_info(self, seq_id):
        bb_anno = jt.var(self.sequence_list[seq_id]["anno"])
        valid = (bb_anno[:, 2] > 0) & (bb_anno[:, 3] > 0)
        # visible = torch.ByteTensor(self.sequence_list[seq_id]['target_visible']) & valid.byte()
        target_visible = [int(v) for v in self.sequence_list[seq_id]["target_visible"]]
        visible = jt.Var(target_visible, dtype=jt.uint8)
        visible &= valid.cast(jt.uint8)
    ...
    def _get_frame(self, sequence, frame_id):
        ...
        frame_path = os.path.join(
            self.root, "Data", "VID", "train", set_name, vid_name, f"{frame_number:06d}.JPEG"
        )
    ...
    def _process_anno(self, root):
        ...
        for set in sorted(os.listdir(base_vid_anno_path)):
            ...
            for vid in sorted(os.listdir(os.path.join(base_vid_anno_path, set))):
                ...
                objects = [
                    ET.ElementTree(file=os.path.join(base_vid_anno_path, set, vid, f)).findall(
                        "object"
                    )
                    for f in anno_files
                ]
