...
from ltr.models.tracking.modal import Modal_loss, modal_swin_fpn
...
def run(settings):
    ...
    settings.description = "Modal with default settings."
    settings.batch_size = 2
    settings.module_name = "Modal_UnFPN"
    ...
    Antiuav_train_fusion = AntiUav_fusion(root="/data01/xjy/code/modal/data/Anti_UAV_RGBT/train")
    ...
    dataset_train = sampler.ModalSampler(
        [Antiuav_train_fusion],
        [1],
        samples_per_epoch=2000 * settings.batch_size,
        max_gap=5,
        processing=data_processing_train,
    )
    loader_train = jt.dataset.DataLoader(dataset_train)
    # The loader for training
    # loader_train = LTRLoader('train', dataset_train, training=True, batch_size=settings.batch_size, num_workers=settings.num_workers,
    #                           shuffle=True, drop_last=True, stack_dim=0)
    print(len(dataset_train))
    # Create network and actor
    model = modal_swin_fpn(settings)
    #
    ...
    #
    actor = actors.ModaltActor(net=model, objective=objective)
    ...
    lr_scheduler = jt.lr_scheduler.StepLR(optimizer, 500)

    # Create trainer
    trainer = LTRTrainer(actor, [loader_train], optimizer, settings, lr_scheduler)
    trainer._checkpoint_dir = "/data01/xjy/code/modal/multi_checkpoint_dir"
