...
class VOT:
    ...
    def _download(self, root_dir, version):
        ...
        for seq in bundle["sequences"]:
            ...
            for cn in channels:
                ...
                seq_file = os.path.join(root_dir, f"{seq_name}_{cn}.zip")
                if not os.path.isfile(seq_file) or md5(seq_file) != seq["channels"][cn]["checksum"]:
            ...
            if not os.path.isfile(anno_file) or md5(anno_file) != seq["annotations"]["checksum"]:
    ...
    def _corner2rect(self, corners, center=False):
        ...
        area1 = np.linalg.norm(corners[:, 0:2] - corners[:, 2:4], axis=1) * np.linalg.norm(
            corners[:, 2:4] - corners[:, 4:6], axis=1
        )
        area2 = (x2 - x1) * (y2 - y1)
    ...
    def _fetch_meta(self, seq_dir, frame_num):
        ...
        # attributes
        tag_files = glob.glob(os.path.join(seq_dir, "*.label")) + glob.glob(
            os.path.join(seq_dir, "*.tag")
        )
        for f in tag_files:
