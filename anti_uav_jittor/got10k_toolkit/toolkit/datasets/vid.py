...
class ImageNetVID:
    ...
    def _cache_meta(self):
        ...
        if "train" in self.subset:
            ...
            anno_dirs_ = [
                os.path.join(self.root_dir, "Annotations/VID/train", *s.split("/")[-2:])
                for s in seq_dirs_
            ]
            seq_dirs += seq_dirs_
        ...
        if "val" in self.subset:
            ...
            anno_dirs_ = [
                os.path.join(self.root_dir, "Annotations/VID/val", s.split("/")[-1])
                for s in seq_dirs_
            ]
            seq_dirs += seq_dirs_
            anno_dirs += anno_dirs_
        ...
        for s, seq_name in enumerate(seq_names):
            ...
            track_ids, counts = np.unique(
                [obj.find("trackid").text for group in objects for obj in group], return_counts=True
            )
