...
class ExperimentLaSOT(ExperimentOTB):
    ...
    def plot_curves(self, tracker_names, extension=".png"):
        ...
        for i, name in enumerate(tracker_names):
            (line,) = ax.plot(
                thr_iou, performance[name][key]["success_curve"], markers[i % len(markers)]
            )
            lines.append(line)
        ...
        for i, name in enumerate(tracker_names):
            (line,) = ax.plot(
                thr_ce, performance[name][key]["precision_curve"], markers[i % len(markers)]
            )
            lines.append(line)
        ...
        for i, name in enumerate(tracker_names):
            ...
            lines.append(line)
            legends.append(
                "%s: [%.3f]" % (name, performance[name][key]["normalized_precision_score"])
            )
        matplotlib.rcParams.update({"font.size": 7.4})
        # legend = ax.legend(lines, legends, loc='center left', bbox_to_anchor=(1, 0.5))
        legend = ax.legend(lines, legends, loc="lower right", bbox_to_anchor=(1.0, 0.0))
