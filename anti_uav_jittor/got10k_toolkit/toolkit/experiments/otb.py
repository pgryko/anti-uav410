...
class ExperimentOTB:
    ...
    def __init__(self, root_dir, version="tb100", result_dir="results", report_dir="reports"):
    ...
    def plot_curves(self, tracker_names):
        ...
        matplotlib.rcParams.update({"font.size": 11})
        ax.set(xlim=(0, 1), ylim=(0, 1), title="Success plots of OPE")
        ...
        for i, name in enumerate(tracker_names):
            (line,) = ax.plot(
                thr_ce, performance[name][key]["precision_curve"], marker=markers[i % len(markers)]
            )
            lines.append(line)
        ...
        matplotlib.rcParams.update({"font.size": 11})
        ax.set(xlim=(0, thr_ce.max()), ylim=(0, 1), title="Precision plots of OPE")
