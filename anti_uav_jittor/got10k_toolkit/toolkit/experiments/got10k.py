...
class ExperimentGOT10k:
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            for r in range(self.repetitions):
                ...
                record_file = os.path.join(
                    self.result_dir, tracker.name, seq_name, "%s_%03d.txt" % (seq_name, r + 1)
                )
            ...
            if save_video:
                video_dir = os.path.join(
                    os.path.dirname(os.path.dirname(self.result_dir)),
                    "videos",
                    "GOT-10k",
                    tracker.name,
                )
                ...
                out_video = cv2.VideoWriter(
                    video_file, cv2.VideoWriter_fourcc(*"MJPG"), 10, (img_W, img_H)
                )
                for ith, (img_file, pred) in enumerate(zip(img_files, boxes, strict=False)):
                    ...
                    if ith < anno.shape[0]:
                        ...
                        cv2.rectangle(
                            img, (gt[0], gt[1]), (gt[0] + gt[2], gt[1] + gt[3]), self.color["gt"], 2
                        )
                    out_video.write(img)
                out_video.release()
            ...
            for name in tracker_names:
                ...
                for s, (_, anno, meta) in enumerate(self.dataset):
                    ...
                    time_file = os.path.join(
                        self.result_dir, name, seq_name, "%s_time.txt" % seq_name
                    )
    ...
    def plot_curves(self, report_files, tracker_names, extension=".png"):
        assert isinstance(report_files, list), (
            'Expected "report_files" to be a list, ' "but got %s instead" % type(report_files)
        )
        ...
        for i, name in enumerate(tracker_names):
            (line,) = ax.plot(
                thr_iou, performance[name][key]["succ_curve"], markers[i % len(markers)]
            )
            lines.append(line)
