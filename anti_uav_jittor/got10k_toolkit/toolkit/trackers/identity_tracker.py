...
class IdentityTracker(Tracker):
    def __init__(self, name, net, window_penalty=0.49, exemplar_size=128, instance_size=256):
        super(IdentityTracker, self).__init__(name=name, net=net, is_deterministic=True)
    ...
    def _convert_score(self, score):
        ...
        score = jt.nn.softmax(score, dim=1).data[:, 0].cpu().numpy()
        return score
    ...
    def init(self, image, box):
        ...
        self.center_pos = np.array([box[0] + box[2] / 2, box[1] + box[3] / 2])
        ...
        z_crop = self.get_subwindow(
            image, self.center_pos, self.exemplar_size, s_z, self.channel_average
        )
        ...
        self.inplace = False
        z_crop[0] = transform.image_normalize(z_crop[0], self.mean, self.std)
    ...
    def update(self, image):
        ...
        x_crop = self.get_subwindow(
            image, self.center_pos, self.instance_size, round(s_x), self.channel_average
        )

        # normalize
        x_crop = x_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        x_crop[0] = transform.image_normalize(x_crop[0], self.mean, self.std)
