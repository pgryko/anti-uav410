...
def send_bbs(bbs):
    global g_logger
    if g_logger:
        g_logger.info(f"send a box : {bbs}")
...
def send_coord(coord):
    ...
    if coord != None:
        ...
        nAimH = coord[3]
        data = struct.pack(
            "iiiiiiii", msgCode, nAimType, nAimX, nAimY, nAimW, nAimH, nTrackType, nState
        )
        # pdb.set_trace()
...
def global_init():
    ...
    if not g_init:
        ...
        IRweights_path = os.path.join(
            "D:\\Project_UAV\\Project_UAV_new_window", "detect_wrapper\\weights\\best.pt"
        )
...
def result_visualization(img, bbox):
    magnification = 2
    oframe = img.copy()
    visuframe = cv2.resize(
        oframe, (oframe.shape[1] * magnification, oframe.shape[0] * magnification), cv2.INTER_LINEAR
    )
