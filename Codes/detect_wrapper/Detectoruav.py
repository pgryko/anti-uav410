...
class DroneDetection:
    def __init__(self, IRweights_path: str, RGBweights_path: str):
        ...
        parser.add_argument(
            "--IRweights", nargs="+", type=str, default=IRweights_path, help="mdel.pt.path(s)"
        )  #'/home/dell/Project/Project_UAV_new/detect_wrapper/weights/best.pt'
        parser.add_argument(
            "--RGBweights", nargs="+", type=str, default=RGBweights_path, help="mdel.pt.path(s)"
        )
        parser.add_argument(
            "--source",
            type=str,
            default="/home/dell/Project_UAV/detect_wrapper/inference",
            help="source",
        )
        ...
        parser.add_argument(
            "--save-dir", type=str, default="inference/output", help="directory to save results"
        )
        parser.add_argument(
            "--classes", nargs="+", type=int, help="filter by class: --class 0, or --clas 0 2 3"
        )
        ...
        _ = (
            self.IR_model(self.IR_img.half() if self.half else self.IR_img)
        ...
        _ = (
            self.RGB_model(self.RGB_img.half() if self.half else self.RGB_img)
            if self.device.type != "cpu"
            else None
        )
    ...
    def forward_IR(self, frame):
        ...
        scale_pred = (
            scale_coords(img.shape[2:], pred[:, :4], frame.shape).round()
