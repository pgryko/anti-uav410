...
cap = cv2.VideoCapture(f"/home/dell/metric_uav/metric_tracker/crops_raw/{video_id}.avi")
anno_file = open(f"/home/dell/metric_uav/metric_tracker/crop_gt/new_video_{video_id}_0.txt")
annotations = anno_file.readlines()
annotations = [list(map(float, annotation.split())) for annotation in annotations]

anno_file_test = open(f"/home/dell/metric_uav/metric_tracker/pytracking/video_{video_id}_0.txt")
...
while g_i < len(annotations) - 1:
    ...
    cv2.rectangle(
        frame,
        (tposition[0], tposition[1]),
        (tposition[0] + tposition[2], tposition[1] + tposition[3]),
        (0, 255, 0),
        2,
    )
