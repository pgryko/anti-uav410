...
def success_count(test: list, gt: list, threshod: float) -> int:
    ...
    for x, y in zip(test, gt, strict=False):
        iou = calIOU(x, y)
        if iou > threshod:
            success_count += 1
            # print(iou)
...
if __name__ == "__main__":
    ...
    for thr in [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8]:
        ...
        for i in range(1, 46):
            main(
                f"/home/dell/metric_uav/metric_tracker/pytracking/video_{i}_0.txt",
                f"/home/dell/metric_uav/metric_tracker/crop_gt/new_video_{i}_0.txt",
                i,
                thr,
            )
